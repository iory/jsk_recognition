...
'''
def resnext50(in_channels=3, pretrained=False, **kwargs):
    """Constructs a ResNet-50 model.
    Args:
        in_channels (int, optional): number of input channels(default: 3)
        pretrained (bool, optional): if True, returns a model pre-trained on Places(default: False)
    """
...
def resnext101(in_channels=3, pretrained=False, **kwargs):
    """Constructs a ResNet-101 model.
    Args:
        in_channels (int, optional): number of input channels(default: 3)
        pretrained (bool, optional): if True, returns a model pre-trained on Places(default: False)
    """
    model = ResNeXt(in_channels, GroupBottleneck, [3, 4, 23, 3], **kwargs)
    if pretrained:
        model.load_state_dict(load_from_url(
            model_urls["resnext101"]), strict=False)
    return model
