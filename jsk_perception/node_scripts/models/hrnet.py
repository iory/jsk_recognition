...
from tasks.utils.io import load_from_url
...
class HRNetV2(nn.Module):
    def __init__(self, in_channels, n_class, is_encoder=False, deep_sup=False, **kwargs):
    ...
    def forward(self, x, segSize=None):
        ...
        if self.is_encoder:
            ...
            x_ = F.interpolate(x_, (height, width),
                               mode="bilinear", align_corners=False)
            return x
        ...
        if self.is_encoder:
            if self.deep_sup:
                ...
                for i in range(len(x)):
                    x[i] = F.interpolate(
                        x[i], size=(x0_h, x0_w), mode="bilinear", align_corners=True)
                return x[::-1]
            return [x[0]]
...
def hrnet(in_channels=3, pretrained=False, **kwargs):
    model = HRNet(in_channels, 150, **kwargs)
    if pretrained:
        model.load_state_dict(load_from_url(
            model_urls["hrnetv2"]), strict=False)
    return model


def hrnetv2(in_channels=3, pretrained=False, **kwargs):
    model = HRNetV2(in_channels, 150, **kwargs)
    if pretrained:
        model.load_state_dict(load_from_url(
            model_urls["hrnetv2"]), strict=False)
    return model
